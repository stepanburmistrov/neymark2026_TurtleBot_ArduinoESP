...
Четвёртый день нужен не для изучения ещё одной отдельной технологии, а для перехода от учебных примеров к собственному проекту.

К этому моменту робот уже умеет:

- двигаться и измерять пройденное расстояние;
- рассчитывать координаты по одометрии;
- принимать команды с компьютера;
- передавать телеметрию и показания датчиков;
- находить цветные объекты;
- распознавать ArUco-маркеры;
- определять положение и направление робота по изображению.

Теперь эти элементы объединяются в законченные сценарии. Каждый пример этого дня может быть использован как основа индивидуального проекта, но максимальная ценность появляется после добавления собственной задачи, новых состояний, датчиков, интерфейса или алгоритма.

---

# Результат дня

К концу четвёртого дня участник должен:

1. повторить получение телеметрии и передачу команд;
2. выбрать основу будущего проекта;
3. запустить хотя бы один пример без изменения архитектуры связи;
4. сформулировать собственную задачу проекта;
5. определить входные данные, состояния и ожидаемый результат;
6. создать репозиторий или отдельную папку проекта;
7. подготовить план реализации на пятый день.
