...
## Полная последовательность второго дня

| Этап | Контрольный результат |
|---:|---|
| 1 | Путь обоих колёс изменяется в миллиметрах и имеет правильный знак |
| 2 | Графики скорости отражают направление и остановку каждого колеса |
| 3 | При ручном перемещении изменяются `X`, `Y` и `θ` |
| 4 | Команды `PING`, `GET`, `SERVO` работают через Serial Monitor |
| 5 | Финальная прошивка принимает `VEL`, удерживает скорость и выдаёт `TEL` |
| 6 | Python управляет роботом по USB и строит траекторию |
| 7 | ESP32-C3 создаёт сеть и передаёт строки между TCP и UART |
| 8 | Тот же интерфейс Python управляет роботом по Wi-Fi |

## Типичные неисправности

| Проявление | Причина или направление проверки |
|---|---|
| Расстояние отличается в постоянное число раз | Неверный `WHEEL_DIAMETER_MM` или `ENCODER_TICKS_PER_REV` |
| Координаты поворота систематически завышены или занижены | Неверный `WHEEL_BASE_MM` |
| Одно колесо имеет отрицательный путь при движении вперёд | Неверный знак соответствующего энкодера |
| Скорость сильно скачет | Слишком короткий период измерения, низкая скорость или нестабильные импульсы |
| Робот уходит с прямой | Требуется уточнение таблиц PWM или коэффициента `STRAIGHT_P_GAIN` |
| Робот не трогается на малой скорости | Команда ниже устойчивого диапазона мотора или недостаточен стартовый PWM |
| После закрытия Python робот останавливается | Нормальная работа watchdog |
| Python не открывает COM-порт | Порт занят Arduino Serial Monitor или выбран другой номер COM |
| После открытия USB нет данных около двух секунд | Нормальный перезапуск Arduino при открытии порта |
| ESP32-C3 создаёт сеть, но команды не доходят | Перепутаны TX/RX, отсутствует общий GND или не совпадает скорость `38400 baud` |
| ESP32-C3 работает нестабильно | Недостаточное питание или подача 5 В непосредственно на GPIO4 без делителя |
| Страница состояния открывается, а Python не подключается | TCP-порт 8888 занят предыдущим клиентом или компьютер подключён к другой сети |
| На карте растёт ошибка после поворотов | Проскальзывание, неточная база или различие диаметров колёс |

## Контрольная точка второго дня

Второй день считается завершённым при выполнении следующих условий:

- путь и скорость обоих колёс рассчитываются в физических единицах;
- положение робота описывается координатами `X`, `Y` и углом `θ`;
- финальная Arduino-прошивка принимает текстовые команды и выдаёт телеметрию;
- движение по USB управляется из Python;
- ESP32-C3 подключена к Arduino с корректным согласованием уровней;
- TCP/UART-мост передаёт команды без изменения протокола;
- управление по Wi-Fi и карта одометрии работают с той же финальной прошивкой Arduino.
